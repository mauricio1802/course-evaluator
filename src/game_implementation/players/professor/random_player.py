...
from ...state import ChallengeDefinition, Challenge, Professor, GameState
from ...game import AddChallengePayload, RemoveChallengePayload
from ....game import Player
from ...utils import challenge_repr

class ProfessorRandomPlayer(Professor, Player):
    ...
    def play(self, states: List[GameState]):
        play_type = random()
        if play_type <= 0.20:
            return Play("finish_turn", None)

        last_state = states[-1] 
        challenges = last_state.challenges

        if play_type < 0.6 and len(challenges) > 0:
            challenge_to_remove = choice(challenges)
            payload = RemoveChallengePayload(challenge_id=challenge_to_remove.get_id())
            return Play("remove_challenge", payload)
        
        players = last_state.players
        score = randint(5000, 20000)
        cost = randint(30, 180)
        interest_required = random()
        challenge_id = self._get_challenge_id()
        player_allowed = choice(players).get_id()

        challenge_definition = ChallengeDefinition(interest_required, cost, score)
        challenge = Challenge(challenge_id, challenge_definition, player_allowed)
        payload = AddChallengePayload(challenge)
        return Play("set_challenge", payload)
