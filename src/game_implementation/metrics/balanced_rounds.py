from ..utils import round_iterator, rounds_window, get_round_number, total_achievable_points_in_round
...
def is_balance(round_to_check, rounds_to_check_against):
    ...
    points_in_rest_rounds = sum(
        map(lambda round: total_achievable_points_in_round(round), rounds_to_check_against))
